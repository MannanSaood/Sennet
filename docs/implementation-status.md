...
|---|---|---|
| SEC-01, SEC-02 | Implemented for new platform | Server-derived tenant, hashed keys, scoped repositories/queries/resources, metadata-only key lists, cross-tenant tests. Shared organization membership and custom roles are not implemented. |
| SEC-03 | Implemented | Expiry/revocation enforced, creation and audit transactional; overlapping keys support rotation. No automatic credential migration. |
...
| SEC-05 | Partial | Raw/decoded limits, gzip limits, partial-signature rejection, deadlines, tenant deduplication. Legacy HMAC lacks nonce/method/path binding; production needs authenticated TLS boundaries. |
