...
- **Control:** short-lived login/workload identity (Firebase/OIDC ID tokens now; mTLS/certificate enrollment remains a future workload option), rotation/revocation, organization/workspace roles, secret-manager references and auditable policy changes. Do not expose user-managed API keys or a shared admin key.
